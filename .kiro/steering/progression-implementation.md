...
- `SceneRenderPipeline`：通过显式 `{ scene, context }` 构造，camera/player/entities/systems/presentation/ui 等正式依赖只从 Context 读取，不再回退到 Scene 同名平铺字段；按 `worldLayers → screenLayers → modalLayers` 有序绘制，render 内禁止更新 UI 状态，Y-sort 缓冲继续复用。显式 `depthSort:true` 的场景图片和 effectZone 粒子进入实体队列，稳定同 Y 顺序为静态图片/装饰（0）→ worldDepth 粒子（1）→实体（2）；普通背景图片与战斗/技能顶层粒子保持原层级。`sortY` 是世界 Y 字段，任何 chunk 投影必须与 `y/points` 同时且只偏移一次。全屏雾、光照等低频柔化层应在低分辨率离屏 Canvas 合成后放大，业务坐标和半径按同一比例投影；单张 SVG 场景背景不得在每个 RAF 直接触发栅格化，也不得在 render 路径同步创建大 Canvas 缓存，必须在资源加载后通过空闲任务预栅格为逻辑分辨率位图并验证旧任务不能覆盖新场景。FrameProfile 必须为默认 screen layer 保留稳定阶段名并继续保留 `renderScreenUi` 聚合值，使点击标记、技能/战斗特效、飘字、教程、对话、战斗 UI、底栏、各按钮、HUD、小地图、战斗状态、转场和性能 HUD 可分别定位；常规 FrameProfile 或性能 HUD 不得隐式安装 Canvas draw-call 方法代理，只有 `setDrawCallTracking(true)` 或显式采样传入 `captureDrawCalls:true` 才允许安装，关闭后下一帧必须恢复 Canvas 原方法。性能 HUD 的 DOM 只按指标刷新周期且值变化时写入，真实性能采样与 HUD 显隐相互独立。
- `SceneCampfireService`：canonical 篝火配置中的 `frameCols × frameRows` 必须覆盖 `frameCount`，绑定图片的自然尺寸必须覆盖 `frameWidth × frameCols` 和 `frameHeight × frameRows`；切换 `imageId` 后先清除旧图，再按当前配置稳定 ID 重新绑定，禁止异步先后顺序保留错图。`particlePresets[].life` 沿用 `Particle` 的毫秒单位，发射 `rate` 和帧 `deltaTime` 使用秒；配置进入运行态前完整校验，失败不得留下半配置。
